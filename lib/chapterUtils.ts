...
export function splitBookIntoChapters(
  ...
  const chapterPatterns = [
    /\bCHAPTER\s+([IVXLCDM]+|\d+)(?:\s+|:|\.)/gi,
    /\bChapter\s+([IVXLCDM]+|\d+)(?:\s+|:|\.)/g,
    /\b([IVXLCDM]+|\d+)\.\s+/g,
    /\n\s*([IVXLCDM]+|\d+)\s*\n/g,
  ];
  ...
  for (let i = 0; i < chapterMarkers.length; i++) {
    ...
    const nextMarker =
      i < chapterMarkers.length - 1 ? chapterMarkers[i + 1] : null;
...
export function getChapterPreview(
  ...
  const contentWithoutHeading = chapterContent.replace(/^.*?\n/, '').trim();
...
export function truncateForAnalysis(
  ...
  const middle = text.slice(
    Math.floor(text.length / 2) - thirdLength / 2,
    Math.floor(text.length / 2) + thirdLength / 2
  );
...
export function calculateOptimalChunkSize(
  ...
  const availableTokens = maxTokens * 0.7;
