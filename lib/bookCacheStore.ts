...
interface BookCacheStore {
  ...
  cachedAnalyses: Record<
    string,
    Record<number, Record<AnalysisType, CachedAnalysis>>
  >;
  recentBooks: string[];
  ...
  persist(
    (set, get) => ({
          ...
          if (estimateCacheSize(newState) > MAX_CACHE_SIZE) {
            ...
            while (
              ...
              if (idToRemove) {
                ...
                newState.recentBooks = newState.recentBooks.filter(
                  (id) => id !== idToRemove
                );
              }
            }
      ...
      getCachedAnalysis: (bookId, chapterIndex, analysisType) => {
        const state = get();
        return (
          state.cachedAnalyses[bookId]?.[chapterIndex]?.[analysisType]
            ?.result || null
        );
      },
      getSavedAnalyses: () => {
